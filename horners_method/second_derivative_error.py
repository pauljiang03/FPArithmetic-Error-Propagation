...
def maximize_horner_error(degree, exp=4, mant=4, max_attempts=50):
    ...
    def compute_custom():
        ...
        for i, coeff in enumerate(coeffs[:-2], 1):
            ...
            first_derivative_mul = fp_mul(first_derivative, x, FP8)
            steps.append((f"first derivative mul {i}", first_derivative_mul))
            first_derivative = fp_sum(first_derivative_mul, val, FP8)
            steps.append((f"first derivative add {i}", first_derivative))

            second_derivative_mul = fp_mul(result, x, FP8)
            steps.append((f"second derivative mul {i}", second_derivative_mul))
            result = fp_sum(second_derivative_mul, first_derivative, FP8)
            steps.append((f"first derivative add {i}", result))

        val = fp_sum(fp_mul(val, x, FP8), coeffs[-2], FP8)
        steps.append((f"value penultimate computation", val))
        first_derivative = fp_sum(fp_mul(first_derivative, x, FP8), val, FP8)
        steps.append((f"first derivative final computation", first_derivative))
        val = fp_sum(fp_mul(val, x, FP8), coeffs[-1], FP8)
        steps.append((f"value final computation", val))
        result = fp_mul(FPVal(2, FP8), result, FP8)
        steps.append((f"second derivative final computation", result))
    ...
    def compute_z3():
        ...
        for i, coeff in enumerate(coeffs[:-2], 1):
            ...
            first_derivative_mul = fpMul(RNE(), first_derivative, x)
            steps.append((f"first derivative mul {i}", first_derivative_mul))
            first_derivative = fpAdd(RNE(), first_derivative_mul, val)
            steps.append((f"first derivative add {i}", first_derivative))

            second_derivative_mul = fpMul(RNE(), result, x)
            steps.append((f"second derivative mul {i}", second_derivative_mul))
            result = fpAdd(RNE(), second_derivative_mul, first_derivative)
            steps.append((f"first derivative add {i}", result))

        val = fpAdd(RNE(), fpMul(RNE(), val, x), coeffs[-2])
        steps.append((f"value penultimate computation", val))
        first_derivative = fpAdd(RNE(), fpMul(RNE(), first_derivative, x), val)
        steps.append((f"first derivative final computation", first_derivative))
        val = fpAdd(RNE(), fpMul(RNE(), val, x), coeffs[-1])
        steps.append((f"value final computation", val))
        result = fpMul(RNE(), FPVal(2, FP8), result)
        steps.append((f"second derivative final computation", result))
