...
mkdir -p value_error_results

# Loop 5 times
i=1
while [ "$i" -le 5 ]; do
    echo "Computing maximum value error for degree $i..."

    # Execute the Python script with the iteration count as an argument
    # Change the second and third
    python3 value_error.py "$i" "$exp_bits" "$mant_bits" > "value_error_results/degree_$i.txt"
    i=$((i + 1))
done